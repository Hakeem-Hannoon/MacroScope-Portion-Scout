...
A versioned SQLite file (~15–30 MB) built by `etl/build-bundle.mjs` and shipped as an app asset:
...
```
node etl/cli.mjs --fdc-dir <dir with FDC csv files> [--out bundle.sqlite] [--priors priors.json]
```
...
- [ ] **Class taxonomy**: map classifier labels → food classes → FDC rows (the label↔row join table is the quality-critical artifact; the store already accepts it as `aliases`)
- [ ] Ship the built bundle as an app asset + implement the on-device expo-sqlite `NutrientStore`
